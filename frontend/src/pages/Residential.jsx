...
const Residential = () => {
  ...
  const filteredProjects = projects.filter(project => {
    // 1. Location Filter (Checks if project location includes the search term)
            ...
            {filteredProjects.map((project) => (
              <div key={project._id} className="bg-white shadow-lg hover:shadow-2xl transition duration-300 group cursor-pointer rounded-lg overflow-hidden flex flex-col h-full animate-fade-in-up">
                ...
                <div className="relative overflow-hidden h-56 sm:h-64 shrink-0">
                  ...
                  <div className="absolute top-4 left-4 bg-yellow-600 text-white text-[10px] sm:text-xs font-bold px-3 py-1 uppercase tracking-wider shadow-md">Featured</div>
                </div>
                ...
                <div className="p-5 sm:p-6 flex flex-col flex-grow">
                  ...
                  <button className="w-full border border-gray-900 text-gray-900 py-3 uppercase text-xs font-bold tracking-widest hover:bg-gray-900 hover:text-white transition active:scale-95">View Details</button>
                </div>
              </div>
            ))}
          </div>
